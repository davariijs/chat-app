...
const InfoRow = ({ icon: Icon, label, value }: { icon: React.ElementType, label: string, value: string }) => (
  <div className="flex items-center gap-3">
    ...
    <div className="min-w-0">
      ...
      <p className="font-medium text-sm md:text-base break-all">{value}</p>
    </div>
  </div>
);
...
export function ContactInfoSidebar({ user, onClose }: { user: User, onClose?: () => void }) {
  return (
    <div className="p-4 h-full overflow-y-auto overflow-x-hidden space-y-6 bg-card ">
      <div className="flex justify-between items-center h-20 border-b -m-4 p-4">
      ...
      <Card className="mt-8 mb-4">
        <CardHeader>
          <CardTitle className="text-center">About Customer</CardTitle>
        </CardHeader>
